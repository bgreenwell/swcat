...
// -- Scroll -------------------------------------------------------------------
// scroll_y is stored in "scroll units"; y_offset = -scroll_y / SCROLL_SCALE
const SCROLL_SCALE: f32 = 60.0;
// Place leading edge just behind the camera (camera sits at Z=10) at startup
...
struct Args {
    ...
    /// Left-align the text instead of centering it (recommended for code)
    #[arg(long)]
    left: bool,
...
async fn main() {
    ...
    // Measure longest line to size textures dynamically.
    ...
    // Pre-render all text into off-screen chunk textures.
    ...
    // Logo render target — populated on the first frame of the Logo state, not here.
    // Rendering the 500px font before next_frame() causes macOS OpenGL to reject the
    // font atlas texture as "unloadable," which breaks all subsequent draw_text calls.
    ...
    loop {
        ...
        match state {
            ...
            State::Crawl => {
                ...
                // Runtime speed control: Up/+ to speed up, Down/- to slow down.
                ...
                // Progress bar: always-visible track + filled portion.
